    ...
    <div class="mb-8 animate-fade-in">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-gray-500 mt-1 text-sm">Manage your personal information.</p>
            </div>
            
            {{-- Role Badge --}}
            <div class="px-4 py-2 rounded-xl border bg-orange-50 border-orange-100 text-orange-700 flex items-center gap-2 shadow-sm">
                <i class="fas fa-id-badge"></i>
                <span class="text-xs font-black uppercase tracking-wide">{{ ucfirst($user->role) }} Account</span>
            </div>
        </div>
    </div>
        ...
        <form action="{{ route('staff.profile.update') }}" method="POST" class="space-y-8">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {{-- Role (Read Only) --}}
                    <div class="group">
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">System Role</label>
                        <input type="text" value="{{ ucfirst($user->role) }}" readonly disabled
                               class="w-full px-4 py-3.5 rounded-xl border border-gray-200 bg-gray-100 text-sm font-bold text-gray-500 cursor-not-allowed">
                    </div>
                </div>
            </div>
            ...
            <div>
                <h3 class="text-sm font-black text-gray-900 uppercase tracking-wider mb-6 flex items-center gap-2">
                    <i class="fas fa-lock text-orange-600"></i> Security (Admin Only)
                </h3>
