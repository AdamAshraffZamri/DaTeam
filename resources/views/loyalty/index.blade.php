                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    @if($voucher->voucherType == 'Merchant Reward')
                                         <p class="text-white/60 text-xs">Use at {{ $voucher->redeem_place }}</p>
                                    @else
                                         <p class="text-white/60 text-xs">{{ $voucher->discount_percent }}% OFF Rental</p>
                                    @endif
                                </div>
                            </div>
                            <div class="text-right">
                                <span class="block text-[10px] font-bold text-white bg-white/10 px-3 py-1 rounded-lg border border-white/20 uppercase mb-1">Valid</span>
                                <span class="text-[9px] text-gray-400">Exp: {{ \Carbon\Carbon::parse($voucher->validUntil)->format('d M') }}</span>
                            </div>
                        </div>
                    @empty
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="bg-white/5 backdrop-blur-2xl rounded-[2.5rem] p-8 border border-white/10 shadow-2xl">
                ...
                <div class="space-y-4">
                    ...
                    @empty
                        <p class="text-gray-400">No rankings yet.</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="bg-white/5 backdrop-blur-2xl rounded-[2.5rem] p-8 border border-white/10 shadow-2xl">
                ...
                {{-- DYNAMIC REWARDS FROM CONTROLLER (INI YANG BETUL) --}}
...
function closeModal() {
    ...
    // Remove old event listeners by replacing onclick
