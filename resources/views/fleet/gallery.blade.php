...
<div class="relative z-10 min-h-screen py-12 pt-28">
    <div class="container mx-auto px-4">
        ...
        @php
            // Filter the collection to show unique brand/model combinations only
            $uniqueFleet = $vehicles->unique(function ($item) {
                return $item->brand . $item->model;
            });
        @endphp

        @forelse($uniqueFleet as $vehicle)
            <div class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl overflow-hidden flex flex-col transition-all duration-500 hover:border-orange-500/50 hover:shadow-[0_0_40px_rgba(249,115,22,0.15)] group">
                ...
                <div class="relative h-64 bg-black/40">
                    ...
                    {{-- Status Badge (Optional: Shows 'Available' if any of this model are available) --}}
                    <div class="absolute top-5 right-5">
                        <span class="bg-emerald-500 text-white px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-xl">
                            Fleet Model
                        </span>
                    </div>
                </div>
